...
class LoginMiddleware
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        $kefuInfo = (new KefuTokenCache())->getKefuInfo($token);

        if (empty($kefuInfo) && !$isNotNeedLogin) {
        ...
        if ($kefuInfo) {
            //获取临近过期自动续期时长
            $beExpireDuration = Config::get('project.kefu_token.be_expire_duration');
            //token续期
            if (time() > ($kefuInfo['expire_time'] - $beExpireDuration)) {
                $result = KefuTokenService::overtimeToken($token);
                //续期失败（数据表被删除导致）
                if (empty($result)) {
