    ...
    <div class="ls-release">
        <el-form label-width="120px" :model="form" :rules="rules" size="small" class="ls-form" ref="form">
            <div class="ls-card ls-release__header">
                <el-page-header @back="$router.go(-1)" content="复制商品"></el-page-header>
            </div>
            ...
            <div class="ls-release__footer bg-white ls-fixed-footer">
                <div class="btns row-center flex" style="height: 100%">
                    ...
                    <el-button v-if="activeName == 'sales'" size="small" type="primary"
                        @click="handleSave">保存</el-button>
                    <el-button v-if="activeName != 'sales'" size="small" type="primary"
                        @click="onNextStep">下一步</el-button>
                </div>
            </div>
        </el-form>
    </div>
    ...
    handleSave() {
        this.$refs.form.validate((valid: boolean, object: any) => {
            if (valid) {
                ...
                let api = null
                delete this.form.id
                const spec_value_list = this.form.spec_value_list.map((item: any) => {
                    // 确保所有必需属性都存在
                    const requiredProps = ['id', 'image', 'sell_price', 'lineation_price', 'cost_price', 'stock', 'volume', 'weight', 'bar_code'];
                ...
                this.form.spec_value_list = spec_value_list;
                
                console.log(this.form, 'from');

                api = apiGoodsAdd(this.form)


                // if (this.isGatherGoods) {
                //     api = apiGatherGoodsEdit(this.form) //采集商品编辑
                // } else {
