...
export default class LsEditor extends Vue {
    ...
    @Prop({ default: false }) decorate!: boolean // 编辑器高度
    @Prop() width!: number
    /** E Props **/
    ...
    valueChange(val: string) {
        // 加载数据时渲染
        if ((this.firstData || this.decorate) && val !== this.editor.txt.html()) {
            this.firstData = false
            this.editor.$textElem.html(val)
        }
    }
    ...
    mounted() {
        ...
        this.editor.create()    
        if (this.firstData && this.decorate) {
            this.firstData = false
