            ...
            <div class="card-content m-t-24">
                <el-row :gutter="20">
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">成交订单</div>
                        <div class="font-size-30">{{ summary.order }}</div>
                    </el-col>
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">订单金额</div>
...
import { apiTransactionAnalysis } from '@/api/data/data'
echarts.use([
    BarChart,
    GridComponent,
    CanvasRenderer,
...
export default class User extends Vue {
    ...
    getDataCenterVisit() {
        ...
        apiTransactionAnalysis({ month: this.month }).then(res => {
            res.order.list[0].data = res.order.list[0].data.reverse()
            res.order_amount.list[0].data = res.order_amount.list[0].data.reverse()

            this.summary = res.summary
            this.business.xAxis.data = res.order.date.reverse()
            this.visitor.xAxis.data = res.order_amount.date.reverse()

            res.order.list.forEach((item: any, index: number) => {
            ...
            res.order_amount.list.forEach((item: any, index: number) => {
