...
class PayConfigValidate extends BaseValidate
{
    ...
    public function checkId($value,$rule,$data)
    {
        ...
        if ($result['pay_way'] == PayEnum::WECHAT_PAY) {
            if (empty($data['interface_version'])) {
                return '微信支付接口版本不能为空';
            }
            if (empty($data['merchant_type'])) {
                return '商户类型不能为空';
            }
            if(! in_array($data['interface_version'], [ 'v2', 'v3' ])) {
                return '微信支付接口版本 必须为v2或v3';
            }
            if (! in_array($data['merchant_type'], [ 'ordinary_merchant', 'partner_sub_merchant' ])) {
                return '商户类型 错误';
            }
            if ($data['interface_version'] == 'v2' && $data['merchant_type'] == 'partner_sub_merchant') {
                return 'v2版本暂不支持子商户';
            }
            if (empty($data['mch_id'])) {
                return '微信支付商户号不能为空';
            }
            if (empty($data['pay_sign_key'])) {
                return '商户API密钥不能为空';
            }
