        ...
        <div class>
            <div class="goods-info bg-white flex">
                ...
                <div class="info-wrap bg-white flex-1">
                    <div class="name bold m-b-16">{{ goodsDetails.name }}</div>
                        ...
                        <div class="flex">
                            ...
                            >
                                <div class="flex row-center count-down xxl">
                                    ...
                                    <div class="white" style="margin: 0 4px">
                                        :
                                    </div>
                                    <div class="item white">
                                        {{ timeData.minutes }}
                    ...
                    <div class="price-wrap lighter">
                        <div class="flex row-between">
                            <div class="price">
                                <div
                                    ...
                                    v-if="
                                        Number(
                                            checkedGoods.lineation_price ||
                                                goodsDetails.lineation_price
                                        )
                                    "
                                >
                        ...
                        <div class="m-t-10 p-b-16" v-if="activity.type != 1">
                            <nuxt-link
                                class="flex row-between"
                                :to="`/get_coupons?id=${goodsDetails.id}`"
                            >
                                <div class="flex col-top">
                                    <div class style="flex: none">优惠</div>
                                    <div
                                        class="coupon-list m-l-20 flex flex-wrap"
                                    >
                                        <div
                                            class="coupon-item xs m-l-10"
                    ...
                    <div class="sales-click muted sm">
                        <template>
                            <span class="m-6">
                                运费：{{
                                    goodsDetails.free_shipping_tips ||
                                    "以提交订单时为准"
                                }}
                            </span>
                        </template>
                        <span v-if="goodsDetails.is_sales_num">｜</span>
                        <span class="m-6" v-if="goodsDetails.is_sales_num">
                            销量：{{ goodsDetails.sales_num }}件
                        </span>
                        <span v-if="goodsDetails.is_click_num">｜</span>
                        <span class="m-6" v-if="goodsDetails.is_click_num">
                            浏览量：{{ goodsDetails.click_num }}次
                        </span>
                    </div>
                    <!-- 规格 -->
                        ...
                        >
                            ...
                            <div class="spec-list flex flex-wrap">
                                <div
                                    ...
                                    :key="sindex"
                                    @click="
                                        onChoseSpecItem(item.id, specitem.id)
                                    "
                                >
                                    {{ specitem.value }}
                                </div>
                    ...
                    <div class="goods-num flex">
                        ...
                        <number-box
                            ...
                            :min="1"
                            :max="
                                goodsDetails.limit_value || checkedGoods.stock
                            "
                        />
                        <div class="m-l-10 sm muted" v-if="goodsDetails.is_stock_show">
                            库存: {{ checkedGoods.stock }}
                        </div>
                        <div
                            class="m-l-20 sm muted"
                            v-if="goodsDetails.limit_type !== 1"
                        >
                            <span v-if="goodsDetails.limit_type === 3"
                                >每笔限购
                                {{ goodsDetails.limit_value }} 件</span
                            >
                            <span v-if="goodsDetails.limit_type === 2"
                                >每人限购
                                {{ goodsDetails.limit_value }} 件</span
                            >
                        </div>
                    ...
                    <div class="goods-btns flex">
                        ...
                        <div
                            ...
                            @click="onAddCart"
                            v-if="activity.type != 1 && goodsDetails.type == 1"
                        >
                            加入购物车
                        </div>
                        <div
            ...
            <div class="details-wrap flex m-t-16">
                ...
                <div class="goods-like">
                    <div class="title flex row-center bg-white">
                        - 猜你喜欢 -
                    </div>
...
export default {
    ...
    async asyncData({ params, $get, app, query }) {
        ...
        // 获取优惠券列表
        let couponLists = [];
        const dataCoupon = await $get("/goods/goodsMarketing", {
            params: {
                id: data.id,
            },
        });
        ...
        if (dataC.code == 1) {
            couponLists = dataCoupon.data.coupon;
    ...
    methods: {
        ...
        onChoseSpecItem(id, specid) {
            ...
            console.log(this.goodsSpec, 123);
        },
        // 加入购物车
        async onAddCart() {
            if (!this.token) {
            ...
            if (this.checkGoodsLimit("cart")) {
                return this.$message({
                    message: `当前商品限购${this.goodsDetails.limit_value}件，已超出！`,
                    type: "error",
                });
            }
        ...
        onBuyNow() {
            ...
            this.$router.push({
                ...
                query: {
                    ...
                    order_type: this.goodsDetails.type == 1 ? 0 : 4, // 订单类型 0-普通订单 1-拼团订单 2-秒杀订单 3-砍价订单 4-虚拟订单
                },
            });
        },
        ...
        async onCollectionGoods() {
            ...
            if (code == 1) {
                if (is_collect == 0) {
                    ...
                    this.goodsDetails.is_collect = 0;
                }
                // this.getGoodsDetail();
            }
        },
        ...
        checkGoodsLimit(event) {
            if (this.goodsDetails.limit_type === 1) return false;

            switch (event) {
                case "cart":
                    if (
                        this.goodsDetails.cart_goods_num + this.goodsNum >
                        this.goodsDetails.limit_value
                    )
                        return true;
                    break;
                case "buy":
                    if (
                        this.goodsDetails.limit_type === 2 &&
                        this.goodsDetails.buy_num + this.goodsNum >
                            this.goodsDetails.limit_value
                    )
                        return true;
                    break;
    ...
    computed: {
        ...
        ...mapState(["token", "publicData"]),
        mobileLink() {
            return `${this.publicData?.shop?.mobile_uri}/pages/goods_detail/goods_detail?id=${this.id}`;
        },
    },
};
</script>
