...
class NoticeEnum
{
    ...
    const RECEIPT_PAYMENT_NOTICE = 111;
    const INTEGRAL_NOTICE = 112;
    const WITHDRAW_CONFIRM_NOTICE = 113;
    ...
    public static function getSceneDesc($sceneId, $flag = false)
    {
        $desc = [
            ...
            self::RECEIPT_PAYMENT_NOTICE => '确认收款通知',
            self::INTEGRAL_NOTICE => '积分到账通知',
            self::WITHDRAW_CONFIRM_NOTICE => '提现待确认收款通知',
        ];
    ...
    public static function getVars($sceneId, $flag = false)
    {
        $desc = [
            ...
            self::RECEIPT_PAYMENT_NOTICE => '用户昵称:nickname 提现金额:withdraw_money 提现时间:withdraw_time',
            self::INTEGRAL_NOTICE => '用户昵称:nickname 订单编号:order_sn 积分:integral',
            self::WITHDRAW_CONFIRM_NOTICE => '用户昵称:nickname 提现金额:withdraw_money 提现时间:withdraw_time',
        ];
    ...
    public static function getSystemExample($sceneId, $flag = false)
    {
        $desc = [
            ...
            self::RECEIPT_PAYMENT_NOTICE => '${nickname}您好！您申请的提现已通过，金额${withdraw_money},请前往商城提现记录中确认收款',
            self::INTEGRAL_NOTICE => '亲爱的{nickname}，您的订单{order_sn}支付成功，获得积分{integral}，已到账。',
            self::WITHDRAW_CONFIRM_NOTICE => '${nickname}您好！您有一笔提现${withdraw_money}待确认收款，请及时前往确认',
        ];
    ...
    public static function getOaExample($sceneId, $flag = false)
    {
        $desc = [
            ...
            self::RECEIPT_PAYMENT_NOTICE => [
                '字段名 字段值 字段内容',
                '提现金额 amount1 {withdraw_money}',
                '提现时间 time2 {withdraw_time}',
            ],
        ];
