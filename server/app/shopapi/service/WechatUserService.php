...
class WechatUserService
{
    ...
    public function getResopnseByUserInfo($source = ''):self
    {
        ...
        $user = User::hasWhere('userAuth',['openid'=>$this->openid],'id,sn,nickname,avatar,mobile,disable,is_new_user')
            ->findOrEmpty();
        /*
        ...
        if($user->isEmpty() && $this->unionid && 'silent' != $source ){
            $user = User::hasWhere('userAuth',['unionid'=>$this->unionid],'id,sn,nickname,avatar,mobile,disable,is_new_user')
                ->findOrEmpty();
        }
    ...
    private function checkAccount()
    {
        if($this->user->disable){
            throw new Exception('您的账号被冻结，请联系客服。');
        }
