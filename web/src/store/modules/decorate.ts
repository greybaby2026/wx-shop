...
import { apiThemePageDetail, apiThemePageEdit, apiPcThemePageEdit, apiPcThemePageDetail } from '@/api/shop'
...
const actions: ActionTree<DecorateState, RootState> = {
     ...
     * @param { Function } commit
     */
    addPages({ state }, { preview }) {
        const { pagesInfo, pagesData } = state
        const addPages =
            state.client == 'mobile'
                ? apiThemePageEdit({
                      ...pagesInfo,
     ...
     * @param { Function } commit
     */
    getPages({ commit, state }, data) {
        const loading = Loading.service({
            lock: true,
            text: '加载中...',
            spinner: 'el-icon-loading'
        })
        const pageDetail = state.client == 'mobile' ? apiThemePageDetail(data) : apiPcThemePageDetail(data)

        return pageDetail
            .then(res => {
                commit('setPagesInfo', res)
                commit('setPagesData', res.content)
